...
TDA_ImagenBMP* recortar(TDA_ImagenBMP* original, int porcentaje) {
    ...
    if (nuevoAncho <= 0 || nuevoAlto <= 0) {
        printf("Error: Las nuevas dimensiones del recorte son inv�lidas.\n");
        return NULL;
    }
    ...
    if (!recortada->matrizDePixeles) {
        printf("Error: No se pudo reservar memoria para la matriz de p�xeles.\n");
        free(recortada);
        return NULL;
    }
...
TDA_ImagenBMP* achicar(TDA_ImagenBMP* original, int porcentaje) {
    ...
    if (!original || porcentaje <= 0 || porcentaje > 100) {
        printf("Error: Imagen nula o porcentaje inv�lido.\n");
        return NULL;
    }
    ...
    if (nuevoAncho <= 0 || nuevoAlto <= 0) {
        printf("Error: Dimensiones finales inv�lidas.\n");
        return NULL;
    }
    ...
    if (!crearMatPixeles(achicada->matrizDePixeles, nuevoAncho, nuevoAlto)) {
        printf("Error al crear la matriz de p�xeles achicada.\n");
